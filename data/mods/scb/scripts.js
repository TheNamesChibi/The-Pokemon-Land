...
maxMoveTable: {
	...
	runMegaEvo(pokemon) {
		...
		// Seraphi Man gains Wonder Guard when Mega Evolving
		if (pokemon.name === 'Seraphi Man' && !pokemon.illusion) pokemon.setAbility('wonderguard');
	...
	modifyDamage(baseDamage, pokemon, target, move, suppressMessages) {
		...
		if (pokemon.status === 'brn' && move.category === 'Physical' && !pokemon.hasAbility('guts') && !pokemon.hasAbility('burningheal') && !pokemon.hasAbility('mutantbody')) {
	...
	pokemon: {
		ignoringAbility() {
			const abilities = [
				'battlebond', 'comatose', 'disguise', 'multitype', 'powerconstruct', 'rkssystem', 'schooling', 'shieldsdown', 'stancechange', 'fossilchange',
			];
		...
		isGrounded(negateImmunity = false) {
			...
			// Innate levitate
			if ((('petunia' in this.volatiles) && !this.illusion) && !this.battle.suppressingAttackEvents()) return null;
		...
		setStatus(status, source = null, sourceEffect = null, ignoreImmunities = false) {
			...
			if (!ignoreImmunities && status.id &&
					!(source && source.hasAbility('maximization') && ['tox', 'psn', 'brn', 'par', 'slp', 'frz'].includes(status.id)) &&
